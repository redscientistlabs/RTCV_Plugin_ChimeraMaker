...
namespace CHIMERA_MAKER
{
    ...
    public class CHIMERA_MAKER : IPlugin, IDisposable
    {
        //-------[ Plugin metadata ]-------
        ...
        public string Name => nameof(CHIMERA_MAKER).Replace("_"," ");
        ...
        public bool Stop()
        {
            if (CHIMERA_MAKER.CurrentSide == RTCSide.Client && !S.ISNULL<PluginForm>() && !S.GET<PluginForm>().IsDisposed)
            {
                S.GET<PluginForm>().HideOnClose = false;
                S.GET<PluginForm>().Close();
            }
